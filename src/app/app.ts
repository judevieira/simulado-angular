...
@Component({
  selector: 'app-root',
  imports: [RouterOutlet, Navbar, Card, Table],
...
export class App {
  ...
  //componente tabela
  protected colunasTabelaJogadores = ['jogador', 'posicao', 'acoes'] 

  //compoente navbar
  protected navItens = [
    {texto: "Home", status: 'active'},
    {texto: "Contato", status: ''},
    {texto: "Sobre nós", status: ''},
    {texto: "Inscreva-se", status: 'disable'}
  ...
  get jogadoresFiltro(){
    ...
    if(this.posicaoFiltros.some(f=> f.selecionado)){
      lista = lista.filter(j => this.posicaoFiltros.some(f=> f.nome == j.posicao && f.selecionado))
    }
